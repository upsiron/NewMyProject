...
FrameBuffer::FrameBuffer(ID3D11Device* device, uint32_t width, uint32_t height)
{
	...
	ID3D11Texture2D *render_target_buffer;
	...
	hr = device->CreateTexture2D(&texture2d_desc, 0, &render_target_buffer);
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));

	D3D11_RENDER_TARGET_VIEW_DESC render_target_view_desc{};
	render_target_view_desc.Format = texture2d_desc.Format;
	render_target_view_desc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
	hr = device->CreateRenderTargetView(render_target_buffer, &render_target_view_desc, RenderTargetView.GetAddressOf());
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	...
	hr = device->CreateShaderResourceView(render_target_buffer, &shader_resource_view_desc, ShaderResourceViews[0].GetAddressOf());
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	...
	hr = device->CreateTexture2D(&texture2d_desc, 0, &depth_stencil_buffer);
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
	...
	shader_resource_view_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	hr = device->CreateShaderResourceView(depth_stencil_buffer, &shader_resource_view_desc, ShaderResourceViews[1].GetAddressOf());
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
